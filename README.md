...
| [`developer`](plugins/developer) | Engineering team | `skill-frontend-web` · `skill-backend` · `skill-sql` · `skill-python` (venv-per-project) · `skill-fastapi` · `ui-ux-pro-max` (design intelligence) · `skill-software-testing` (test + UAT) · *(subagent-fullstack + subagent-qa-tester — Phase B)* |
