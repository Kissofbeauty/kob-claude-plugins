...
| [`developer`](plugins/developer) | Engineering team | `skill-frontend-web` (HTML/CSS/SCSS/Tailwind/JS/TS/React) · `skill-backend` (API/server design) · `skill-sql` (schema/query/security) · `skill-software-testing` (test + UAT) · *(subagent-fullstack + subagent-qa-tester เร็ว ๆ นี้ — Phase B)* |
